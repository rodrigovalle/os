...
#[repr(u8)] // each variant stored as a u8
pub enum Color {
    Black = 0,
    Blue = 1,
    Green = 2,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(transparent)] // use same layout as u8
struct ColorCode(u8);
...
fn test_println_many() {
    // screen is 80x25, anything past 25 here will invoke scrolling
    for _ in 0..200 {
...
fn test_println_output() {
    ...
    let s = "Some single line test string";
    println!("{}", s);
    for (i, c) in s.chars().enumerate() {
        let screen_char = WRITER.lock().buffer[0][i].read();
        assert_eq!(char::from(screen_char.ascii_character), c);
    }
}

#[test_case]
fn test_print_long_lines() {
    for _ in 0..200 {
        print!("a");
    }
}
